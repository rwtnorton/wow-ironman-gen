...
let choose (rand:System.Random) choices =
    List.randomChoiceWith rand choices

let chooseWowClassByRace (rand:System.Random) (race:Race) =
    race
    |> allowedWowClassesByRace
    |> choose rand

let chooseFactionByRace (rand:System.Random) (race:Race) =
    race
    |> allowedFactionsByRace
    |> choose rand

let chooseGenderByRace (rand:System.Random) (race:Race) =
    race
    |> allowedGendersByRace
    |> choose rand

let chooseRace (rand:System.Random) =
    allRaces
    |> choose rand
...
type Toon = {
    ...
    override this.ToString () =
        ...
        if factionCnt > 1 then $"{this.Gender} {this.Faction} {this.Race} {this.WowClass}"
        else $"{this.Gender} {this.Race} {this.WowClass}"
    member this.isValid () =
